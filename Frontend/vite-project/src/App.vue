<script setup>
  ...
  const getStudents = () => {
    ...
    .then(res => {
      books.splice(0, books.length); // 把旧的数据清空
      books.push(...res.data.results);  // 把新的数解包
      console.log("更新数据");
    })
  ...
  /*表单添加*/
  const add_dialog_visible = ref(false); // 控制表单显示
  const ruleFormRef = ref() // 放表单的对象引用
  ...
  const submitForm = (formEl) => {
    ...
    axios.post('http://localhost:5000/books/',book_form)
    .then(() => {
